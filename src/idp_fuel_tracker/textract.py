...
def filename_filter(f):
    return any(f.name.lower().endswith(x) for x in ["jpg", "jpeg"]) and not any(
        f.name.lower().endswith(x) for x in ["annotated.jpg", "annotated.jpeg"]
    )
...
async def create_annotated_image(doc, image_path: Path, overwrite: bool = False):
    ...
    annoted_image_path = image_path.parent / image_path.name.replace(
        image_path.suffix, ".annotated" + image_path.suffix
    )
...
async def process_documents(files: List[Path] = [], overwrite=False):
    print(files)
